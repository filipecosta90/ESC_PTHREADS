threads_csv = readtable('THREAD_DTRACE_mutex_4_0_8192_4.csv','ReadVariableNames',false);
...
ylabel('Thread ID (Thread 0 = Processo Unix)');
xlabel('Tempo (ms)','interpreter','latex');
t = title({'Rela\c{c}\~ao entre cria\c{c}\~ao de fios de execu\c{c}\~ao e tempo total em mili-segundos para m\''etodo MUTEX, 4 POSIX Threads,\newline','PERSONAL LAPTOP, intervalo [0:8192], n=4, para compilador gcc 4.9.0 sem flags de otimiza\c{c}\~ao de compila\c{c}\~ao'},'interpreter','latex')
